...
static struct typeinfo *get_left_type(struct node *n);
static struct typeinfo *get_right_type(struct node *n);

static bool get_public(struct node *n);
static bool get_private(struct node *n);

static void symbol_insert(char *k, struct typeinfo *v, struct node *t,
                          struct hasht *l, bool constant);
static bool handle_node(struct node *n, int d);
static void handle_init(struct typeinfo *v, struct node *n);
static void handle_init_list(struct typeinfo *v, struct node *n);
static void handle_function(struct typeinfo *t, struct node *n, char *k);
static void handle_class(struct typeinfo *t, struct node *n);
...
struct tokenitem *get_category(struct node *t, int target, int before)
{
    struct tokenitem *token = get_category_(t, target, before);
...
struct tokenitem *get_category_(struct node *t, int target, int before)
{
    log_assert(t);
    
    if (tree_size(t) == 1) {
        struct tokenitem *token = get_token(t->data);
    ...
    while (!list_end(iter)) {
        struct tokenitem *token = get_category_(iter->data, target, before);
        if (token && (token->category == target
...
entryptr new_entry(char* n) {
   ...
   // entryptr e = calloc(1, sizeof(*e));
    typeptr t = calloc(1, sizeof(*t));
...
tableptr new_table(char* n) {
   ...
   // int i;
   // tableptr t = calloc(1, sizeof(*t));
    
    tableptr t = calloc(1, sizeof(struct table));
    t->entry[1] = calloc(1, sizeof(struct entry));
...
bool lookup(char *n, tableptr t) {
     ...
     // unsigned long key = sdbm(n) % 10000;
    int key = get_key(n);
...
char *class_member(struct node *t)
{
...
struct node *get_production(struct node *n, enum rule r)
{
    ...
    while (!list_end(iter)) {
        struct node *t = get_production(iter->data, r);
        if (t)
            return t;
        iter = iter->next;
    }
...
{
    struct tokenitem *token = get_category(t, CLASS_NAME, IDENTIFIER);
    if (token)
        return token->text;
    else
        return NULL;
...
int get_array(struct node *t)
{
    if (get_category(t, '[', INTEGER)) {
        struct tokenitem *token = get_category(t, INTEGER, ']');
...
char *get_identifier(struct node *n)
{
    ...
    for (i = 0; i < sizeof(types) / sizeof(types[0]); ++i) {
        struct tokenitem *t = get_category(n, IDENTIFIER, -1);
        if (t)
            return t->text;
    }
...
static bool get_public(struct node *t)
{
    return get_category(t, PUBLIC, PRIVATE);
}

static bool get_private(struct node *t)
{
 ...
 */
static struct typeinfo *get_typeinfo(struct node *t) {
    log_assert(t);
...
struct typeinfo *type_check(struct node *n)
{
    log_assert(n);
    if (tree_size(n) == 1)
    ...
    switch (production) {
        ...
        case NEW_EXPR: {
            ...
            /* new operator */
            struct node *type_spec = get_production(n, TYPE_SPEC_SEQ);
            if (type_spec == NULL)
            ...
            if (type->base == CLASS_T) {
                ...
                struct node *expr_list = get_production(n, EXPR_LIST);
                if (expr_list) {
        ...
        case POSTFIX_CALL: {
            ...
            r->function.parameters = list_new(NULL, NULL);
            struct node *iter = get_production(n, EXPR_LIST);
        ...
        case SHIFT_RIGHT: {
            ...
            struct node *l = child(0);
            struct node *r = child(1);
        ...
        case FUNCTION_DEF: {
            ...
            struct node *jump = get_production(n, RETURN_STATEMENT);
            struct typeinfo *ret = NULL;
