...
static void token_realloc_sval(struct tokenitem *t);
static void token_realloc_text(struct tokenitem *t, const char *s);
/*
void print_token(tok_1 t) {
...
tok_1 create_token(int category, char *text, int lineno, char *filename) {
    tok_1 t = malloc(sizeof(struct tokenitem));
...
{
    struct tokenitem *t = malloc(sizeof(*t));
    if (t == NULL)
 ...
 * null characters.
 */
void token_finish_sval(struct tokenitem *t)
{
    token_sval_size = 0;
