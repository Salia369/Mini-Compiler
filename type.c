 ...
 */
struct typeinfo *typeinfo_new(struct node *n)
{
	log_assert(n);
	...
	if (get_rule(n) == CTOR_FUNCTION_DEF) {
		...
		struct tokenitem *token = get_token(iter->data);
 ...
 */
struct typeinfo *typeinfo_new_array(struct node *n, struct typeinfo *t)
{
...
struct typeinfo *typeinfo_new_function(struct node *n, struct typeinfo *t, bool define)
{
